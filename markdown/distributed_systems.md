...
7. Distributed Mutual Exclusion Lock Algorithm
...
8. Distributed Mututal Exclusion Lock Message Complexity
...
2. Components of RPC Latency
    * Client call: Client sets up arguments, calls kernel, marshals data
    * Controller latency: NIC DMAs message into buffer and sends
    * Time on wire: Transmission protocols take some amount of time
    * Interrupt handling: OS must handle message receipt
    * Server setup to execute call: Unmarshals, moves data, calls procedure
    * Server execution + reply: Time depends on specific procedure
    * Client setup to receive results and restart: Reverse of sending process
3. Sources of Overhead on RPC
    * Marshaling
    * Data Copying
    * Control Transfer
    * Protocol Processing
    * Reducing overhead: Take advantage of what hardware gives you
4. Marshaling and Data Copying
...
5. Control Transfer
    * Context switches that must happen to affect an RPC call
        - 4 switches
            + Client to kernel (network call here)
            + Kernel to server
            + Server to kernel (network call here)
            + Kernel to client
...
8. Pros and Cons of Active Networks
    * Pros:
        - Flexibility from application perspective
    * Cons:
        - Protection threads (prevent interference with other network flows)
            + ANTS runtime safety -> Java sandboxing
            + Code spoofing -> robust fingerprint (hash)
            + Soft state integrity -> restricted API
        - Resource management threads (prevent executing code from going wild)
...
1. Systems from Components Introduction
...
2. The Big Picture
    * Design cycle
        ...
        - Deploy
3. Digging Deeper from Specification to Implementation
...
4. Putting the Methodology to Work
    ...
    * Getting to an unoptimized OCaml implementation
        ...
        - Mimic VLSI design
5. How to Optimize the Protocol Stack
