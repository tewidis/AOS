...
1. Shared Memory Machine Models
    * Dance hall architecture: CPUs on one side, memory on other side of
...
| Dance Hall Architecture |
    ...
    * Symmetric Multiprocessor: Bus connects CPUs to memory

| ![symmetric](images/shm_symmetric_multiprocessor.png) |
|:--:|
| Symmetric Multiprocessor Architecture |

    * Distributed Shared Memory: Each CPU has nearby memory, but all memory
...
4. Memory Consistency and Cache Coherence
...
3. Scalability Issues with Synchronization
...
4. Native Spinlock (spin on test\_and\_set)
    * lock(L):  
    ...
    * Problems with Native Spinlock
        ...
        useful work)
5. Caching Spinlock (spin on read)
    * Spin on value in local cache, then try test\_and\_set when unlocked
    * lock(L):
        - while( L == locked );
...
6. Spinlock with Delay
    * Delay after lock release
    * while( (L == locked) || (test\_and\_set(L) == locked) )
        - while( L == locked );
        - delay( d[process_id] );
    * Only works with hardware cache coherence
    * Delay with exponential backoff
    * while( test\_and\_set(L) == locked )
        - delay(d);
        - d = d * 2;
...
7. Ticket Lock
    ...
    * struct lock{ int next\_ticket; int now\_serving; };
    * acquire\_lock(L):
...
8. Spinlock Summary
    * read and test\_and\_set (no fairness)
    * test\_and\_set with delay (no fairness)
    * ticket\_lock (fair but noisy)
9. Array-based Queueing Lock
    ...
    * Only one atomic operation (fetch\_and\_increment)
...
11. Algorithm Grading
    * If processor provides a fetch\_and\_phi operation, queueing locks are
...
3. Tree Barrier
    ...
    * Issues
        - locksense variable is dynamically determined by arrival pattern
        - Lots of contention if number of processes is large
        - If not cache coherent, the spin occurs on remote memory
...
4. MCS Tree Barrier (4-ary Arrival Tree)
...
2. RPC vs Local Procedure Call
    * In a local call, everything happens at compile time
    * RPC:
        - Caller traps into kernel (call trap)
        - Kernel validates call
        - Kernel copies arguments into kernel buffers from client address 
        space
        - Kernel locates server procedure
        - Kernel copies arguments from kernel buffer to address space of
        the server
        - Kernel schedules server to run the procedure
        - When server is complete, it traps again (return trap)
        - Kernel copies results from address space of server to kernel 
        buffers
        - Kernel copies results from kernel buffers to client address space
        - Kernel schedules client to continue running
...
4. Reducing RPC Binding Overhead
...
6. Reducing RPC Overhead Summary
...
1. How should the scheduler choose the next thread to execute?
    * FCFS: Most fair
...
5. Minimum Intervening Plus Queue Policy
...
6. Summarizing Scheduling Policies
...
9. Cache Affinity and Multicore
...
1. OS Challenges for Parallel Machines
...
2. OS Design Principles for Parallel Machines
...
4. Parallel OS and Page Fault Service
...
5. Recipe for Scalable Structure in Parallel OS
    * For every subsystem:
        - Determine the functional needs of that service
        - To ensure concurrent execution of service, minimize shared data
        structures (less sharing -> more scalable)
        - Where possible, replicate/partition system data structures. This
        allows for less locking and more concurrency
6. Tornado;s Secret Sauce
...
8. Objectization of Memory Management
...
9. Objectized Structure of VM Manager
...
10. Advantages of Clustered Object
...
11. Implementation of Clustered Object
...
12. Non Hierarchical Locking and Existence Guarantee
...
16. Summary of Ideas in Corey System
